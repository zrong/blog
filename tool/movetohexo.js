...
const flashRe =  /\[kml_flashembed .+\]/gi
...
function checkPluginFlash (content) {
...
function updateFrontMatter (lines, filename) {
    ...
    // 处理 ForontMatter
    for (var i=0; i<lastFrontMatterLine; i++) {
        var line = lines[i]
        ...
        // nickname 不是英文的情况，删除
        if (key == 'nicename') {
            if(noValueRe.test(value)) continue
        }
        // wp 中的附件
...
function updatePlugin (lines, filename) {
    for (var i=0; i<lines.length; i++) {
        var line = lines[i]
        /* 检测 Flash 插件，需要支持多行，改用 updatePluginFlash
        var flashParams = checkPlugin(flashRe, line)
        if (flashParams) {
            logger.log('Flash plugin %s found.', filename)
            logger.log('flashParam:%j', flashParams)
            line = `{% flash %}${os.EOL}${yaml.safeDump(flashParams)}{% endflash %}`
            lines[i] = line
            logger.log('flash line: %s', line)
        }
        */
        // 检测下载插件
