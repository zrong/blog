...
原来的代码如果一字不改，在进行加密的时候会报错：

```
TypeError: IV is not meaningful for the ECB mode
```

这是因为 IV 仅应用于 `MODE_CBC` , `MODE_CFB` , `MODE_OFB` 和 `MODE_OPENPGP` 。在 [AES 文档][aes] 中有说明。
...
关于 ECB 模式并不是语义安全的说法，在这里有详细介绍： [Why shouldn't I use ECB encryption?][ecb] 。

不过在这里我们就不深究安全性的问题了。去掉 IV：
