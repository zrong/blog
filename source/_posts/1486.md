...
[Flex编译器参数中-swf-version与-target-player之关系](http://blog.zengrong.net/post/1486.html)
...
* <span style="color:red">2012-03-20更新：</span>本篇文章只讲了Flex SDK的情况，但Flash IDE也能通过修改配置文件的方式支持新的Flash Player功能，详见这里：[让Flash支持更新的Flash Player功能](http://blog.zengrong.net/post/1568.html)
...
记得在[Flash Player 10.2发布](http://blog.zengrong.net/post/1244.html)的时候，为了使用Flash 10.2提供的原生位图鼠标光标功能，需要在编译的时候将`-swf-version`编译器属性值设置为11。以此推算，14这个值是针对Flash Player 11.1的。
...
（这里是基于Flex SDK和Flash Builder讲解，如果你使用Flash Professional，可以看这里：[让Flash支持更新的Flash Player功能](http://blog.zengrong.net/post/1568.html)）

1. 在程序编写期间，Flash Builder使用它来提供自动完成功能。当然，如果直接用mxml编译器（[比如我](http://blog.zengrong.net/post/1307.html)），就没多大关系；
