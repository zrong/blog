...
本篇是归因系列五篇文章中的最终篇。我们来聊聊广告ID的前世今生，政府出台的法案对个人隐私保护的意义。<!--more-->
...
![Allow Tracking](/upload/2021/03/allow_tracking.jpg)

公布了ATT后，苹果公司并没有立刻启用它，但 2020 年末苹果高管发言称，在2021年春季，苹果公司将会全面推行ATT，如果应用开发者拒绝接入该功能，则会被从 App Store 中强制删除对应 App。

根据我的几个开发者朋友的反馈，他们的 APP 在 2021 年 2 月提审到 AppStore 时被拒审，原因是 APP 中包含的 SDK 有将 IDFA 上传到第三方服务器。从这一点看来，苹果对 IDFV 的管控会越来越严。
...
1. 没有实时回传。新的归因方式意味着回传最快也要发生在安装之后的 24 小时，如果运气不好（例如经常重置倒计时），回传发生在数月之后……
2. 匿名数据问题。SKAdNetwork 的数据采用聚合形式，不提供用户层级的精细数据，难以进行精细的 ROI 优化；
3. 数据量不易使用。SKAdNetwork 提供的是 6 bit 事件识别码，识别码只能单向变化和向上递进。
4. 数据量不足。SKAdNetwork 对每个渠道仅显示 100 个不同的推广活动。在对多个国家/地区进行多素材测试的时候是不够用的。这会减少投放上 Campaign 的数量设置；
5. 回传浪费。SKAdNetwork 先把数据传回广告平台，广告平台再将数据传给归因平台。这意味着归因平台（无论是自建还是第三方）都需要和每个广告主进行对接，这是巨大的技术资源浪费。

{{< alert >}}
要了解现有的归因流程，可以阅读 [广告归因：归因方法][how]。
{{</ alert >}}
...
- [AppsFlyer：面向 SKAdNetwork 的一站式解决方案 SK360：新增预测分析和防作弊两大利器][sk360]
...
一个优秀的程序员，应该致力于用自动化的方式去解决所有可以自动化的问题。优秀的程序员，必须同时是一个优秀的产品经理。
