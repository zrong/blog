...
本文是 [从 Flask 到 Gin —— 目录](/post/flask-to-gin-index/) 系列的一部分。

----

如果写过 [C++](/tag/cpp/) 或者 [Java](/tag/java/)，你会觉得 JSON 的处理在 [Golang](/tag/golang/) 中比前两者要简单很多。但作为习惯了偷懒写 [Python](/tag/python/) 的我来说，Golang 中的 JSON 用法还是让人挺难受的。 

[MJP](/tag/mjp/) 是一个 RESTful API 服务，绝大多数 API 返回的都是 JSON 格式。由于 Python 的灵活性和 Flask 的良好封装，编写 MJP 服务时，我在 JSON 上没有遇到什么困难。我在 MJP 中封装了一个方法 `responseto` 用来统一输出 JSON Response。本文介绍将这个方法移植到 Gin 的过程。 <!-- more --> 


## responseto 的 Python 实现

`responseto` 是一个 Python 模块方法，它的定义如下：
...
上面的源码注释已经很清晰了。[jsonify][jsonify] 是 Flask 提供的一个 JSON Response 封装，它会返回一个 JSON 响应。有必要多说一句的是 `db.to_response_data` 。

这个方法的作用是把 SQLAlchemy 的 Model 对象（一般是数据库查询的结果）转换成为一个 dict，进而会使用 jsonify 方法将其转换成为 JSON Response。因为与本文关系不大，这里就不贴出 `db.to_response_data` 的源码了。
...
从上面的例子可以看出，得益于 Pyhon 方法的 kwargs 参数机制，`responseto` 的使用是非常灵活的。可以省略掉许多参数，也可以动态调整返回的 JSON 内容的键名。

这些灵活的手段，在 Golang 中会遇到挑战。

## responseto 在 Golang 中的挑战

在 Golang 中实现 `responseto` 方法，至少会碰到 3 个问题。
...
查看源码，gin 已经为我们准备了一些封装。 `gin.H(utils.go)` 这个封装提供了一个类似于 `Python dict` 的结构。而 `Context.JSON(context.go)` 提供了类似于 `Flask jsonify` 的方法。
...
## Responseto/ResponsetoWithData/ResponsetoWithAllData

由于 Golang 不提供方法重载和关键字参数，我创建了 3 个方法来对应 `responseto`：
...
上面的三个方法层层递进，提供了 Flask 版本 responseto 能提供的大部分功能。要完全实现 Flask 版本的功能，需要定义更多的方法，或者采用 Golang 中的变长参数。那会对方法的使用者产生困扰。我认为上面 3 个方法即保证了抽象足够简洁，也能保持一定的使用灵活性。

下面是一个 `Responseto` 和 `ResponsetoWithData` 在路由中使用的例子：
...
和上面那个 Flask 路由的例子类似，第一个 `Responseto` 方法提供了一个包含 `error/code/message` 键名的 JSON 对象，第二个 `ResponsetoWithData` 方法提供了一个带有具体数据返回的 JSON 对象。
...
也许你注意到了上面代码中的 `models.ActiveSerializer` 这个名称，是的，序列化我们必须自己来做。

由于 Golang 的语言特色（Public 必须大写字母开头），在定义数据库字段的时候，你必须要进行一些映射。另外，你也并不一定希望所有的数据库字段都返回给客户端，这个自定义的过程，就需要手动序列化。
...
需要注意 `ActiveSerializer` 和 `ActivesSerializer` 的区别。它们分别用来返回 **一个** `ActiveModel` 对象和 **一组** `ActiveModel` 对象。在 Python 这类动态语言中，我们可以将它们放在同一个方法中，在 Go 中就必须分开处理。

如果只想简单把 JSON 转换成 Struct 定义，可以使用这个网站： [JSON-to-Go](https://mholt.github.io/json-to-go/)。
