...
上篇文章我们讨论过，uWGI 的 `req-logger` 和 `logger` 两个参数是插件化的 logger，它们不支持 `logreopen` 功能。但也正是因为它们的插件化功能，让他们可以支持多个 logger 写入数据源。
...
- 文档中提到的 `zeromq` logger 插件在 PyPI 最新版本中没有提供，因此不可用。

我认为比较高效的方式，是使用 `redislog` 来分发日志，我需要写一个独立的日志接收服务，对发来的日志进行分类存储和集中化管理。
...
- 提供命令行来启动服务。
- 支持使用 [Supervisor][supd] 守护 pyzog 服务。
- 提供 [systemd][systemd] 配置文件来守护 [Supervisord][supd]。
...
pyzog 需要一个守护，保证进程 crash 的时候可以自动拉起。我选择使用 [Supervisor][supd]。它已经在安装 pyzog 的时候一起安装了。
...
Supervisor 是一个进程管理器，需要在服务器启动的时候自动启动。在 Supervisor 的文档 [Running supervisord automatically on startup](http://supervisord.org/running.html#running-supervisord-automatically-on-startup) 中可以找到对应操作系统的 [initscripts](https://github.com/Supervisor/initscripts)。

目前，许多 Linux 发行版都已经使用 systemd 替换了标准的 initscript。我们这里使用 systemd 来代替老旧的 initscript。

pyzog 可以创建 systemd 配置文件。确保当前处于 `~/pyzog` 文件夹中，且位于虚拟环境的激活状态：
